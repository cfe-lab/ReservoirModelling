# Focusing on the individuals whose earliest bin is "short" (i.e. the bin 
# representing the incomplete earliest year of their infection, when counting
# backwards from ART initiation by year), perform the model-free analysis but
# glomming together these people's two earliest bins.
...
half.lives <- NULL  # a data frame to keep track of all of the computed half-lives

# Identify people whose earliest bin would be short.
first.bin.lengths <- integration.data[match(unique(integration.data$pid), integration.data$pid), c(1, 3)]
first.bin.lengths$first.year <- first.bin.lengths$untreated.period / 365
first.bin.lengths$first.year <- first.bin.lengths$first.year - trunc(first.bin.lengths$first.year)
pids.short.first.year <- first.bin.lengths$pid[first.bin.lengths$first.year < 0.5]
...
for (pid in names(all.log.likelihoods[[vl.info]])) {
    ...
    for (col.date in names(all.log.likelihoods[[vl.info]][[pid]])) {
        ...
        if (pid %in% pids.short.first.year) {
            # Each of these people has a short first year, so glom together the earliest two bins
            # (i.e. the ones furthest back in time).  Treat this entire bin as what would have been
            # the "second-earliest" bin, which is now the earliest bin.
            # Example: if this person's untreated period was 3.25 years, then we will now have bins 
            # representing
            # 0-1 years
            # 1-2 years
            # 2-3.25 years
            breakpoints <- c(breakpoints[1:(length(breakpoints) - 2)], breakpoints[length(breakpoints)])
        }
        ...
        cairo_pdf(
            paste0(
                "model_free_combined_earliest_years_", 
                pid,
                "_",
                col.date,
                ".pdf"
...
pdf("half_lives_combined_earliest_years.pdf")
...
# We need special handling for the individuals with upper bounds that are infinite,
# and also for Z326M, whose upper bound is much larger than max.y but finite.
...
z326m.row <- plot.1.data$pid == "Z326M"
z326m.idx <- which(z326m.row)
arrows(
    x0=which(!(upper.bound.inf | z326m.row)),
    y0=plot.1.data$half.life[!(upper.bound.inf | z326m.row)],
    y1=plot.1.data$half.life.upper[!(upper.bound.inf | z326m.row)],
    angle=90,
    length=0.1,
    code=2,
    lwd=1
...
# For Z326M, make something fancier.
segments(
    x0=z326m.idx,
    y0=plot.1.data$half.life[z326m.idx],
    y1=inf.upper.y - 2,
    lwd=1
)
segments(
...
text(
    ...
    labels=paste0(
        "upper\nbound\n",
        round(plot.1.data$half.life.upper[z326m.idx], digits=2)
    ),
    cex=1.5,
    pos=4
)
...
pdf("half_lives_two_sampling_points_combined_earliest_years.pdf")
par(mar=c(11.5, 6.5, 2, 2) + 0.1)
plot(
    x=c(1, dna.x),
    y=c(0, max.y),
