# This analysis is akin to ode_based_likelihood_plots.r, but
# using the integration data that includes duplicates.
...
for (vl.info in c("known", "typical")) {
    for (pid in names(all.log.likelihoods[[vl.info]])) {
        for (col.date in names(all.log.likelihoods[[vl.info]][[pid]])) {
            ...
            pdf(paste("lls_with_duplicates_", pid, "_", col.date,  "_", vl.info, "_vl.pdf", sep=""))
