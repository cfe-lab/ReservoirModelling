...
for (subject in subjects) {
    ...
    subject.data <- read.csv(paste(subject, "sample_times.csv", sep="/"))
...
for (subject in subjects) {
    ...
    ode.solutions.bin.30[[subject]] <- undecayed.reservoir.distribution.given.vl(
        grafted.vls[[subject]]$time,
        grafted.vls[[subject]]$viral.load,
        infection.date,
        art.initiation,
        bin.size=30,
        grid.size=0.001
...
save.image("elite_controllers_known_vl.RData")
...
for (subject in subjects) {
    ...
    pdf(paste(subject, "_known_vl.pdf", sep=""))
    plot(
        possible.half.lives,
        lls,
        main=paste("LLs (using known VL) by decay rate:", subject),
